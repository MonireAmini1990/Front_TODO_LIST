      ...
      <div className="max-w-md mx-auto my-6 w-full flex-1">
        {filteredTasks.length === 0 ? (
          ...
          filteredTasks.map((task) => {
            // subtasks مستقیماً آرایه است
            const subtasksArray = Array.isArray(task.subtasks) ? task.subtasks : [];
            return (
              <div key={task.task_id} className="bg-white rounded-lg shadow p-4 mb-4 space-y-2">
                    ...
                    {subtasksArray.length > 0 && (
                      <ul className="list-disc ml-6 mt-2 space-y-1 text-gray-600">
                        {subtasksArray.map((sub, idx) => (
                          <li key={`${task.task_id}-sub-${idx}`} className="text-sm">{sub}</li>
                        ))}
                      </ul>
                    )}
                  </div>
